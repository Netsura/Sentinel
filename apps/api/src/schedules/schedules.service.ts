...
export class SchedulesService {
  ...
  async create(user: AuthUser, workspaceId: string, dto: CreateScheduleDto) {
    ...
    const schedule = await this.prisma.scheduledScan.create({ data: { workspaceId, assetId: asset.id, createdById: user.id, mode: dto.mode, frequency: dto.frequency, nextRunAt } });
  ...
  async pause(user: AuthUser, workspaceId: string, scheduleId: string) {
    ...
    return this.prisma.scheduledScan.update({ where: { id: schedule.id }, data: { enabled: false } });
  }
  ...
  async resume(user: AuthUser, workspaceId: string, scheduleId: string) {
    ...
    return this.prisma.scheduledScan.update({ where: { id: schedule.id }, data: { enabled: true, nextRunAt: this.nextRun(schedule.frequency) } });
  }
  ...
  async remove(user: AuthUser, workspaceId: string, scheduleId: string) {
    ...
    const schedule = await this.find(user, workspaceId, scheduleId);
    await this.queue.removeRepeatable('scheduled-scan', { every: this.interval(schedule.frequency), jobId: `schedule:${schedule.id}` });
    await this.prisma.scheduledScan.delete({ where: { id: schedule.id } });
    return { success: true };
  }
